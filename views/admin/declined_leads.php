...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'resend') {
        if ($registrationController->resendToRegistration($lead_id)) {
            header('Location: /std_mgmt/views/admin/declined_leads.php?course=' . urlencode($course) . '&user_id=' . urlencode($user_id) . '&success=Lead resent for registration successfully');
            exit;
        } else {
        ...
        if ($leadController->updateLeadDetails($lead_id, $permanent_address, $work_experience)) {
            header('Location: /std_mgmt/views/admin/declined_leads.php?course=' . urlencode($course) . '&user_id=' . urlencode($user_id) . '&success=Lead details updated successfully');
            exit;
        } else {
...
<form method="GET" action="/std_mgmt/views/admin/declined_leads.php">
...
<?php if ($course && !empty($declined_leads)): ?>
    ...
    <table class="table">
        ...
        <tr>
            ...
            <td>
                <form method="POST" action="/std_mgmt/views/admin/declined_leads.php?course=<?php echo urlencode($course); ?>&user_id=<?php echo urlencode($user_id); ?>">
            ...
            <td>
                <form method="POST" action="/std_mgmt/views/admin/declined_leads.php?course=<?php echo urlencode($course); ?>&user_id=<?php echo urlencode($user_id); ?>" style="display: inline;">
                    <input type="hidden" name="lead_id" value="<?php echo htmlspecialchars((string)$lead['lead_id']); ?>">
